...
\sub{热力学三定律}
    ...
    热力学第一定律$\d U=\d Q+\d W=\d Q-P\d V$\\
    热力学第二定律$\d Q \leq T\d S \ip \d U \leq T\d S -P\d V$。
\sub{温度定义}
...
\sub{热容定义}
    $C=\dd{Q}{T}$, 定压热容比$C_p=(\pp{Q}{T})_p=(\pp{H}{T})_p$, 定容热容比$C_V=(\pp{U}{T})_V=(\pp{H}{T})_V$, 迈耶公式 $C_p-C_V=nR$
\sub{几个常见过程}
    准静态过程：气体外界做功$\d W=-p\d V$。对磁性体系$\d W=H\d M$, 对电学体系$\d W=E\d P$, 化学反应粒子数变化$\d W=\mu\d N$.\\
    ...
    等压过程：$\d H=\d U+p\d V=\d Q$\\
    等容过程：$\d H=\d U+V\d p,\ \d U=\d Q$
\sub{几个常见体系}
    孤立体系：无能量，物质交换，无体积变化。\\
    封闭体系：无物质交换。\\
    开放体系：有物质交换。
\sub{四个热力学基本量}
    ...
    温度 $T$ 理想气体内能只与温度有关。\\
    热 $\d Q = T\d S$\\
    熵 对于可逆过程，熵$S$是一个态函数，积分与路径无关；对于绝热过程中，熵永不减少。$\d S=\f{\d Q}{T},\ S=\f{3}{2}Nk\ln (pV^{5/3})+const,\ S=k\ln\o$ \\
    $T,P,S,V$ 中只有两个独立变量。
\se{均匀物质的热力学性质}
\sub{四个势函数}
    \def({\lparen} 
    \def){\rparen}
    $$\ar[rrlrllr]{
        \text{内能} & U= & Q+W & \d U= & T\d S-P\d V & \d U_{S,V} \leq 0 \ & -(\pp{P}{S})_V=(\pp{T}{V})_S\\
        \text{焓} & H= & U+pV & \d H= & T\d S+V\d P & \d H_{S,p} \leq 0 & (\pp{T}{P})_S=(\pp{V}{S})_p\\  
        \text{亥姆霍兹自由能} & F= & U-TS & \d F= & -S\d T-P\d V & \d F_{T,V} \leq 0 & (\pp{S}{V})_T=(\pp{p}{T})_V\\
        \text{吉布斯自由能} & G= & U-TS+pV \ & \d G= & -S\d T+V\d P \ & \d G _{p,T} \leq 0 & -(\pp{S}{p})_T=(\pp{V}{T})_p
    }$$
