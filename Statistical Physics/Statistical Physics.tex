...
\sub{四个势函数}
    ...
    $$\ar[rrlrllr]{
        \text{内能} & U= & Q+W & \d U & =C_V\d T + \left[T(\pp{p}{T})_V-p\right]\d V  & =T\d S-P\d V & -(\pp{P}{S})_V=(\pp{T}{V})_S\\
        \text{焓} & H= & U+pV & \d H  & =C_p\d T+\left[V-T(\pp{V}{T})_p\right]\d p & =T\d S+V\d P & (\pp{T}{P})_S=(\pp{V}{S})_p\\  
        \text{自由能} & F= & U-TS & \d F & \d F_{T,V} \leq 0 & =-S\d T-P\d V & (\pp{S}{V})_T=(\pp{p}{T})_V\\
        \text{吉布斯} & G= & U-TS+pV & \d G & \d G _{p,T} \leq 0 & =-S\d T+V\d P & -(\pp{S}{p})_T=(\pp{V}{T})_p\\
        \text{熵} & S &  & \d S & =\f{C_V}{T}\d T+(\pp{p}{T})_V\d V\\
